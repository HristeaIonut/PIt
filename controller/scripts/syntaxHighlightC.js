...
console.log(codeElements);
    ...
    hashTagReg = /(#)/g,
    specialReg = /\b(auto|case|continue|do|default|const|else|if|for|goto|return|static|switch)(?=[^\w])/g,
    variableTypeReg = /\b(int|char|double|float|signed|unsigned|long|short|struct|union|void)(?=[^\w])/g,
    functionsReg = /\b(printf|scanf)/g,
    //specialCommentRegWithLineBreaks = /\*.*\n.*\*\//gm,
    //specialCommentRegWithoutLineBreaks = /\*.*?\*/gm,
    inlineCommentReg = /(\/\/.*)/g;
    //librariesReg = /\<(.*?)\>/g;
    ...
    codeElements = codeElements.replace(specialCommentRegWithLineBreaks,'<span class="special-comment">$1</span>');
    //codeElements = codeElements.replace(specialCommentRegWithoutLineBreaks,'<span class="special-comment">$1</span>');
